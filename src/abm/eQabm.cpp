...
//            double mutantScale = double(eQ::data::parameters["mutantAspectRatioScale"]);
//            size_t pointsToDecimate = //expected # points for the mutated cell
//                    round ((( thisCell->getLengthMicrons() * mutantScale) - 1.0)
//                            * 2.0*2.0);//hard code 2 nodes-per-micron (deined below)

//            auto newCellPoints = std::vector<eQ::nodePoint>{};
//            if(pointsToDecimate >= cellPoints.size())
//            {
//                newCellPoints = cellPoints;
//            }
//            else
//            {
//                auto pointsBuffer = cellPoints;
//                for (size_t i(0); i<pointsToDecimate; ++i)
//                {
//                    size_t which = floor(rn() * pointsBuffer.size());
//                    newCellPoints.push_back(pointsBuffer[which]);
//                    pointsBuffer.erase(pointsBuffer.begin() + which);
//                }
//            }

//            for(auto &point : newCellPoints)
        ...
        {//NOTE:  HSL passed in is in units of concentration [nM]...must scale to extra-cellular volume to ensure convservation of molecule #
            ...
            double updatePerSquareMicron = eQ::Cell::nanoMolarPerMoleculePerCubicMicron
                    * numberHSL/eQ::Cell::computeExtraCellularVolumeFraction(cellLength);
...
//                    else
        ...
        //for simulating how many points are used for a smaller cell:
//        {
//            double mutantScale = double(eQ::data::parameters["mutantAspectRatioScale"]);
//            size_t pointsToDecimate = //expected # points for the mutated cell
//                    round ((( thisCell->getLengthMicrons() * mutantScale) - 1.0)
//                            * 2.0*2.0);//hard code 2 nodes-per-micron (deined below)
//            averagePointsPerCell += pointsToDecimate;
//        }
